...
class EnergyData(BaseModel):
    ...
    power_kw: float = Field(..., ge=0, le=100, description="Power consumption in kW")
    voltage_v: float = Field(..., ge=200, le=250, description="Voltage in Volts")
    current_a: float = Field(..., ge=0, le=50, description="Current in Amperes")
    frequency_hz: float = Field(..., ge=49, le=51, description="Frequency in Hz")
...
class EnergyProfile(BaseProfile):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        pattern = self.load_patterns.get(self.load_pattern, self.load_patterns["residential"])
