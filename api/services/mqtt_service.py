...
class MQTTService:
        ...
        for name, profile_class in profiles.items():
            try:
                ...
                profile_info = ProfileInfo(
                    name=name,
                    description=profile_class.__doc__
                    or f"{name.title()} data simulation profile",
                ...
                self.logger.warning(
                    f"Error getting info for profile {name}: {e}"
                )
    ...
    def get_profile_info(self, profile_name: str) -> Optional[ProfileInfo]:
        ...
        try:
            ...
            return ProfileInfo(
                name=profile_name,
                description=profile_class.__doc__
                or f"{profile_name.title()} data simulation profile",
                parameters=parameters,
                example_topic=example_topic,
                example_payload=example_data,
            )
            ...
            self.logger.error(
                f"Error getting profile info for {profile_name}: {e}"
            )
            return None
    ...
    def generate_profile_preview(
        ...
        try:
            ...
            self.logger.error(
                f"Error generating preview for {profile_name}: {e}"
            )
            return None
    ...
    def start_simulation(self, config: SimulationConfig) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Started simulation with profile: {config.profile.name}"
            )
            return True
    ...
    def stop_simulation(self) -> bool:
        ...
        try:
            ...
            if (
                self.simulation_state.thread
                and self.simulation_state.thread.is_alive()
            ):
    ...
    def get_simulation_status(self) -> SimulationStatus:
        ...
        return SimulationStatus(
            ...
            profile_name=self.simulation_state.config.profile.name
            if self.simulation_state.config
            else None,
            topic=self.simulation_state.client.profile.get_topic()
            if self.simulation_state.client
            and self.simulation_state.client.profile
            else None,
            interval=self.simulation_state.config.interval
            if self.simulation_state.config
            else None,
            messages_sent=self.simulation_state.messages_sent,
            start_time=self.simulation_state.start_time.isoformat()
    ...
    def get_simulation_data(self, limit: int = 50) -> Dict[str, Any]:
        ...
        messages = (
            self._message_history[-limit:] if self._message_history else []
        )
        result = {
            "messages": messages,
    ...
    def _run_simulation(self, duration: Optional[int] = None):
        ...
        try:
            ...
            while not self.simulation_state.stop_event.is_set():
                ...
                # Publish data
                if (
                    self.simulation_state.client
                    and self.simulation_state.client.is_connected
                ):
                    ...
                    if success:
                        ...
                        if self.simulation_state.client.profile:
                            data = (
                                self.simulation_state.client.profile.generate_data()
                            )
                            ...
                            message_record = {
                                ...
                                "topic": (
                                    self.simulation_state.client.profile.get_topic()
                                ),
                                "payload": data,
                            }
                ...
                interval = (
                    self.simulation_state.config.interval
                    if self.simulation_state.config
                    else 5
                )
