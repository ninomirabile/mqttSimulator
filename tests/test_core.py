...
class TestMQTTClient:
    ...
    def test_mqtt_client_auto_client_id(self):
        ...
        assert match1 is not None, f"Client ID {client1.client_id} doesn't match expected pattern"
        assert match2 is not None, f"Client ID {client2.client_id} doesn't match expected pattern"
    ...
    def test_set_profile(self):
        ...
        client = MQTTClient()
        profile = WeatherProfile(city="Test City")
    ...
    @patch('mqtt_simulator.core.time.sleep')
    @patch('mqtt_simulator.core.mqtt.Client')
    def test_start_simulation_with_duration(self, mock_mqtt_client, mock_sleep):
        ...
        with patch('mqtt_simulator.core.time.time') as mock_time:
            mock_time.side_effect = [0, 1, 6]  # Start time, then after duration
