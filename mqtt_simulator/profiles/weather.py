...
class WeatherData(BaseModel):
    ...
    temperature: float = Field(
        ..., ge=-50, le=60, description="Temperature in Celsius"
    )
    humidity: float = Field(
        ..., ge=0, le=100, description="Humidity percentage"
    )
    wind_speed: float = Field(
        ..., ge=0, le=200, description="Wind speed in km/h"
    )
...
class WeatherProfile(BaseProfile):
    ...
    def generate_data(self) -> Dict[str, Any]:
        ...
        temperature = self.add_randomness(
            self.base_temp + temp_variation, 0.15
        )
