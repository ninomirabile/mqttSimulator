...
class MQTTConfig(BaseModel):
    ...
    retained: bool = Field(default=False, description="Retain message for new subscribers")
    last_will_topic: Optional[str] = Field(default=None, description="Last will topic")
    last_will_message: Optional[str] = Field(default=None, description="Last will message")
    last_will_qos: QoSLevel = Field(default=QoSLevel.AT_MOST_ONCE, description="Last will QoS")


class ProfileConfig(BaseModel):
    """Profile configuration for simulation."""
    name: str = Field(..., description="Profile name (weather, agriculture, energy, etc.)")
    parameters: Dict[str, Any] = Field(default_factory=dict, description="Profile-specific parameters")
    topic: Optional[str] = Field(default=None, description="Custom MQTT topic (optional)")
...
class SimulationConfig(BaseModel):
    ...
    interval: int = Field(default=5, ge=1, le=3600, description="Publishing interval in seconds")
    duration: Optional[int] = Field(default=None, ge=1, description="Simulation duration in seconds")


class SimulationStatus(BaseModel):
    """Current simulation status."""
    is_running: bool = Field(..., description="Whether simulation is currently running")
    is_connected: bool = Field(..., description="Whether connected to MQTT broker")
    profile_name: Optional[str] = Field(default=None, description="Current profile name")
    topic: Optional[str] = Field(default=None, description="Current MQTT topic")
    interval: Optional[int] = Field(default=None, description="Current publishing interval")
    ...
    last_message: Optional[Dict[str, Any]] = Field(default=None, description="Last published message")
...
class ProfileInfo(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(..., description="Available parameters and their types")
...
class SimulationData(BaseModel):
    ...
    messages: List[Dict[str, Any]] = Field(..., description="Recent published messages")
