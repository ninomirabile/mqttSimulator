...
@cli.command()
@click.option(
    "--profile", "-p", required=True, help="Simulation profile to use"
)
...
@click.option(
    "--interval", "-i", default=5, help="Publishing interval in seconds"
)
...
@click.option(
    "--soil-type",
    type=click.Choice(["sandy", "clay", "loam"]),
...
def run(
    ...
    if profile not in available_profiles:
        ...
        click.echo(
            f"Available profiles: {', '.join(available_profiles.keys())}"
        )
        sys.exit(1)
...
def list_profiles():
    ...
    click.echo()
    click.echo(
        "Use 'mqtt-simulator run --profile <name>' to run a simulation."
    )
